...
def calculate_expected_loss(control_simulation, treatment_simulation, treatment_won, min_difference_delta=0.02):
...
def plot_simulations(file_name):
    ...
    for i in range(1, no_of_simulations+1):
        ...
        if i == 1:
            control = ax1.plot(filtered['sample'], filtered['control_expected_loss'], label='control', linewidth = 0.05, color = 'b')
            treatment = ax1.plot(filtered['sample'], filtered['treatment_expected_loss'], label='treatment', linewidth = 0.05, color = 'r')
            threshold = ax1.plot(filtered['sample'], np.full((10000,), 0.0015), linewidth = 0.02, label='threshold', color = 'g')
            ...
            ax1.plot(filtered['sample'], filtered['control_expected_loss'], linewidth=0.02, color = 'b', alpha=0.3)
            ax1.plot(filtered['sample'], filtered['treatment_expected_loss'], linewidth=0.02, color = 'r', alpha=0.3)
    ...
    plt.savefig('wtfplot.png', dpi = 1000)
    plt.close()


plot_simulations('experiment_simulations_37.csv')

standard_simulations = run_multiple_experiment_simulations(100, 1, 1, 0.3, 0.32, 0.001)
standard_simulations.to_csv('experiment_simulations_1.csv', index=False)

simulations = pd.read_csv('experiment_simulations_1.csv')
myfilter = simulations[simulations['treatment_expected_loss'] > (0.001)]
myfilter.quantile([0.75,0.8,0.85,0.9], axis = 0)


simulations = pd.read_csv('experiment_simulations_37.csv')
myfilter = simulations[simulations['control_expected_loss'] < 0.0015]
myfilter.to_csv('myfilter.csv', index = False)
x = max(myfilter['sample'])


myfilter2 = simulations[simulations['treatment_expected_loss'] > (0.05*0.048)]
myfilter2.quantile([0.75,0.8,0.85,0.9], axis = 0)
myfilter2.to_csv('myfilter2.csv', index = False)
